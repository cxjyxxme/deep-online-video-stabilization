    ...
    try:
        while(True):
            ...
            for j in range(args.refine):
                start = time.time()
                img, black, Hs = sess.run([output, black_pix, Hs_tensor], feed_dict={x_tensor:tmp_in_x})
        ...
        max_s = 0
        ans = []
        for i in range(0, int(math.floor(height * 0.3)), 10):
            print(i)
            print(max_s)
            for j in range(0, int(math.floor(width * 0.3)), 10):
                if (all_black[i][j] > 0):
                    continue
